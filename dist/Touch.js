...
var elementsFromPoint = (function (x, y) {
  ...
  // var docElement = document.elementFromPoint(x,y)
  var shadowRoots = Array.prototype.slice.call(document.getElementsByTagName('*'), 0).filter(element => element.shadowRoot).map(el => el.shadowRoot)

  var elAtPoint = function() {
    return document.elementFromPoint(x,y) || shadowRoots.map(e => e.elementFromPoint(x,y)).flat().find(e => e != undefined)
  }

  var all = [...document.elementsFromPoint(x,y), ...shadowRoots.map(e => e.elementsFromPoint(x,y)).flat()]
  console.log("ALL",all)


  // if there's an element and it's not in the list
  // while ((current = elAtPoint())) {
  ...
  _createClass(TouchBackend, [{
    ...
    value: function connectDropTarget(targetId, node) {
      ...
      var handleMove = function handleMove(e) {
         ...
         */
        var root = node.getRootNode() === document ? document : node.getRootNode();
        var droppedOn = root.elementFromPoint(coords.x, coords.y);
        var childMatch = node.contains(droppedOn);

        console.log("node", node)
        console.log("droppedOn", droppedOn)
        console.log("childMatch", childMatch)
        console.log("equals", droppedOn === node)
    ...
    value: function handleMove(e, targetId) {
      console.log("adding drag over target id")
    ...
    value: function handleTopMove(e) {
      ...
      console.log("drag over target ids", dragOverTargetIds)
      var dragOverTargetNodes = dragOverTargetIds.map(function (key) {
        return _this4.targetNodes[key];
      }); // Get the a ordered list of nodes that are touched by
      console.log("drag over target nodes", dragOverTargetNodes)

      var elementsAtPoint =
          this.getDropTargetElementsAtPoint
          ? this.getDropTargetElementsAtPoint(clientOffset.x, clientOffset.y, dragOverTargetNodes)
          : elementsFromPoint(clientOffset.x, clientOffset.y); // Extend list with parents that are not receiving elementsFromPoint events (size 0 elements and svg groups)
      ...
      console.log("extended", elementsAtPointExtended)
      ...
      console.log("orderedDragOverTargets", orderedDragOverTargetIds)
